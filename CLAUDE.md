...
| `/api/queue/stop` | GET/POST | 终止当前处理中的任务（ffprobe 验证音频完整性） |
...
| 2026-04-16 | 残留任务（downloading/transcribing）重启后无法恢复 | `cleanup_stale_tasks()` 启动时直接删除 |
...
| 2026-04-17 | 多 tab 打开导致状态混乱 | 加单页保护 `vtf_page_active` |
...
Flask 启动
    │
    ▼
cleanup_stale_tasks()          ← downloading/transcribing 直接删除
    │
    ▼
...
ffprobe 验证音频完整性
    │
    ├─ 音频不完整 → _task_terminated=True，worker finally 杀子进程，DB 改 pending
    └─ 音频完整 → 弹框（[继续] / [暂停] / [重置] + 右上角 X）
                    ├─ [继续] → 任务继续
                    ├─ [暂停] → _task_terminated=True，db.pause_episode()，保留音频
                    └─ [重置] → _task_terminated=True，db.reset_episode_for_retry()，删音频
```
...
对外：`get_current_task_info()`、`get_current_audio_file()`、`set_task_terminated()`、`is_task_terminated()`
...
| 残留任务 | 需手动清理 | `cleanup_stale_tasks()` 自动删除 |
